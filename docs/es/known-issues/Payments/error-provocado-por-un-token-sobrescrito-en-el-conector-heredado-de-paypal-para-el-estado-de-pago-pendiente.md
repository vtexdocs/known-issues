...
En nuestro conector heredado de PayPal, nos hemos encontrado con un problema relacionado con la sobrescritura del campo `TOKEN` para `PAYMENTINFO_0_TRANSACTIONID`. El problema surge cuando el proveedor envía un estado de pago como «pendiente». En consecuencia, el TOKEN queda sobrescrito por la implementación de nuestro conector, lo que provoca un error generado por PayPal durante las solicitudes posteriores. Como resultado, la transacción se queda bloqueada con un mensaje de «token no válido».
