---
title: 'El reescritor no recibe actualizaciones de ruta del catálogo.'
...
Algunas actualizaciones del catálogo relacionadas con las rutas no llegan al reescritor. Esto afecta a marcas, categorías, subcategorías y departamentos, en ambas direcciones:

- Las **rutas nuevas o editadas** no son recibidas por el reescritor. Normalmente, ejecutar la consulta de arranque es suficiente para sincronizarlas, pero en casos particulares no funciona. En la tienda online, la ruta no devuelve productos o muestra el error "algo se ha descompuesto".
...
Una señal útil para el segundo caso: compare la ruta en `pageType` y en el reescritor. Si `pageType` devuelve `fullText` mientras que el reescritor sigue devolviendo el tipo antiguo, significa que `pageType` se actualizó y el reescritor no.
...
Después de cualquiera de las mutaciones, vuelva a ejecutar el reescritor. Se realiza una consulta `internal.get` para confirmar que la ruta se resuelve correctamente.