---
title: 'El descuento nominal calculado según la fórmula no funciona como se esperaba en las operaciones de resta'
slug: el-descuento-nominal-calculado-segun-la-formula-no-funciona-como-se-esperaba-en-las-operaciones-de-resta
status: PUBLISHED
createdAt: 2023-08-24T13:30:21.000Z
updatedAt: 2023-08-28T20:32:12.000Z
contentType: knownIssue
productTeam: Pricing & Promotions
author: 2mXZkbi0oi061KicTExNjo
tag: Pricing & Promotions
slugEN: nominal-discount-based-on-formula-not-working-as-expected-with-subtraction-operations
locale: es
kiStatus: Backlog
internalReference: 886980
---

>ℹ️ Este problema conocido ha sido traducido automáticamente del inglés.

## Sumario

Al crear una promoción utilizando un descuento nominal basado en una fórmula, el comerciante puede realizar las siguientes operaciones según esta documentación: https://help.vtex.com/en/tutorial/promocao-regular-com-desconto-nominal-baseado-em-formula--2Pwrq6THyGViNedQG381jV
 ![](https://vtexhelp.zendesk.com/attachments/token/1EhryrRZDsiXZ31kaSV2VcegW/?name=image.png)

Sin embargo, al utilizar la operación de resta, el valor del descuento se sustituye por el valor total del pedido.

## Simulación

Crea una promoción utilizando el descuento nominal basado en fórmula y utiliza una operación de resta, de la siguiente manera:
 ![](https://vtexhelp.zendesk.com/attachments/token/3wIEiPGtF9oX2daeQ3F3AZqCk/?name=image.png)

Comprueba en la caja que el descuento se sustituye por lo que debería ser el valor total del pedido:
 ![](https://vtexhelp.zendesk.com/attachments/token/r2bWktL4reNcVnnhSqr4hT0FC/?name=image.png)

## Workaround

N/A