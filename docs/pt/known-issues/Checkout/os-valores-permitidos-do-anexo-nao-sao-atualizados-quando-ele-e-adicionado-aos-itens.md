---
title: 'Os valores permitidos do anexo não são atualizados quando ele é adicionado aos itens'
slug: os-valores-permitidos-do-anexo-nao-sao-atualizados-quando-ele-e-adicionado-aos-itens
status: PUBLISHED
createdAt: 2023-10-17T20:16:26.000Z
updatedAt: 2023-10-17T20:16:26.000Z
contentType: knownIssue
productTeam: Checkout
author: 2mXZkbi0oi061KicTExNjo
tag: Checkout
slugEN: attachments-permitted-values-are-not-updated-when-its-added-to-items
locale: pt
kiStatus: Backlog
internalReference: 920970
---

>ℹ️ Este problema conhecido foi traduzido automaticamente do inglês.

## Sumário

Os valores permitidos de um anexo não são atualizados se ele tiver sido adicionado a um item antes da alteração desses valores no painel de administração. Isso causará um erro no carrinho com a mensagem “Não foi possível se comunicar com o vendedor”.

## Simulação

- Crie um anexo com valores permitidos;
- Adicione um item com o anexo;
- Altere os valores permitidos do anexo;
- Atualize o carrinho.

## Workaround

N/A