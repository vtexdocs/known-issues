...
Às vezes, podem ocorrer erros durante o processamento de nossas operações pelo gateway. No caso do conector da Firstdata, pode ocorrer um erro imediatamente após a aprovação da liquidação por nossa parte. Para resolver isso, precisamos solicitar uma liquidação novamente. No entanto, devido à idempotência, o provedor não envia uma resposta “aprovada” novamente, o que pode fazer com que a transação fique presa no status “em liquidação” até que uma nova atualização de status seja fornecida.

## Simulação

Não é possível reproduzir o erro, pois ele ocorre de forma intermitente.
