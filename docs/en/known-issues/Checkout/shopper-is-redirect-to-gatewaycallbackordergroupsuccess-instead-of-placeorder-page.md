...
status: PUBLISHED
createdAt: 2025-12-02T18:56:51.035Z
updatedAt: 2025-12-02T18:56:51.035Z
...
When the shopper makes a purchase using a gift card together with a payment app, if the payment has not yet been approved or denied when the app is closed, the shopper is redirected to the page
...
4. If the payment is not approved yet, the shopper will receive JSON similar to:
  1.
...
Ensure that the connector only triggers `$(window).trigger('transactionValidation.vtex', [status])` after the payment is actually approved.
